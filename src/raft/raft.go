...
func (rf *Raft) readPersist(data []byte) {
	...
	if d.Decode(&logs) != nil {
		Debug(dPersist, "S%d", "S%d: failed to readPersist: logs")
	} else {
		rf.logs = logs
	}
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotRPCArgs, reply *InstallSnapshotRPCReply) {
	...
	if args.Term < rf.currentTerm {
		Debug(dSnap, "S%d: the Leader's term(%d) is outdated.", rf.me, args.Term)
	...
	if args.LastIncludedIndex <= rf.lastIncludedIndex {
		...
		Debug(dSnap, "S%d: the leader's snapshot is outdated", rf.me)
		return
	}
	...
	if args.LastIncludedIndex >= rf.logs.Len() {
		Debug(dSnap, "S%d's logs is slow", rf.me)
	...
	if args.LastIncludedIndex > rf.logs.StartIndex && args.LastIncludedIndex < rf.logs.Len() {
		Debug(dSnap, "S%d: the snapshot have a prefix of its log", rf.me)
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	if index <= rf.lastIncludedIndex {
		Debug(dSnap, "S%d: the snapshot generated by service is outdated", rf.me)
		return
	}
	if index > rf.commitIndex {
		Debug(dSnap, "S%d: the logs haven't commited", rf.me)
		return
	}
	...
	Debug(dSnap, "S%d: Before Cutting Log the logs: %v, startIndex: %d", rf.me, rf.logs.Entries, rf.logs.StartIndex)
	rf.logs.CutStart(index + 1)
	rf.persist()
	Debug(dSnap, "S%d: After Cutting Log, the logs: %v, startIndex: %d", rf.me, rf.logs.Entries, rf.logs.StartIndex)
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	defer rf.mu.Unlock()
	if args.Term < rf.currentTerm {
		Debug(dAppe, "S%d have a higher term %d", rf.me, rf.currentTerm)
		reply.Success = false
		reply.Term = rf.currentTerm
		return
	}
	...
	if args.PrevLogIndex >= rf.logs.Len() {
		...
		Debug(dAppe, "S%d's logs is short", rf.me)
		reply.Success = false
		...
		if args.PrevLogIndex == rf.lastIncludedIndex {
			if args.PrevLogTerm != rf.lastIncludedTerm {
				Debug(dWarn, "S%d: the snapshot's last included term is inconsistent with Leader", rf.me)
				panic("fails")
			}
		} else {
			if args.PrevLogIndex+len(args.Entries) > rf.lastIncludedIndex {
				Debug(dWarn, "S%d: the prevLogIndex %d is less than Follower's StartIndex %d, entries: %v", rf.me, args.PrevLogIndex, rf.logs.StartIndex, args.Entries)
				...
				Debug(dWarn, "S%d: Log entries fully covered by the snapshot", rf.me)
	...
	*/
	...
	if args.LeaderCommit > rf.commitIndex {
		...
		Debug(dCommit, "S%d: process the commandIndex %d", rf.me, rf.commitIndex)
	}
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.currentTerm {
		// args.Term < rf.currentTerm
		Debug(dVote, "S%d have higher term %d for Candidate %d", rf.me, rf.currentTerm, args.CandidateId)
		reply.Term = rf.currentTerm
	...
	if !updateToDate {
		Debug(dVote, "S%d: the Candidate %d's logs is not update to date", rf.me, args.CandidateId)
		reply.VoteGranted = false
		return
	}

	Debug(dVote, "S%d grand the vote request from Candadate %d", rf.me, args.CandidateId)
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.persist()
	index = rf.logs.Len() - 1
	Debug(dClient, "S%d receive an command %v at index %d", rf.me, command, index)
...
func (rf *Raft) CovertToFollower(term int) {
	Debug(dCtoF, "S%d: Convert to Follower", rf.me)
	rf.currentTerm = term
	rf.state = Follower
	rf.votedFor = -1
	rf.persist()
...
func (rf *Raft) CovertToLeader() {
	Debug(dCtoL, "S%d: Convert to Leader", rf.me)
	rf.state = Leader
	rf.persist()
...
func (rf *Raft) sendAppend(server int) {
	...
	go func() {
		Debug(dInfo, "S%d: send Append Entry RPC to S%d", rf.me, server)
...
func (rf *Raft) sendSnapShot(server int) {
	...
	go func() {
		Debug(dSnap, "S%d send InstallSnapShot RPCs to S%d", rf.me, server)
...
func (rf *Raft) requestVote(server int, arg *RequestVoteArgs, votes *int) {
	...
	Debug(dInfo, "S%d: send Request Vote to S%d", rf.me, server)
...
func (rf *Raft) tick() {
	...
	if rf.state == Leader {
		...
		if time.Now().After(rf.electionTime) {
			Debug(dTimer, "S%d: Election Timer timeout", rf.me)
			rf.startElection()
		}
	}
}
...
func (rf *Raft) apply() {
	for {
		...
		if rf.lastIncludedIndex > rf.lastApplied {
			...
			Debug(dSnap, "S%d: successfully apply a snapshot", rf.me)
			...
			for rf.commitIndex > rf.lastApplied {
				...
				Debug(dAppl, "S%d: successfully apply msg", rf.me)
				msgs = append(msgs, msg)
			}
		}
		rf.mu.Unlock()
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	Debug(dInfo, "S%d: Create a Raft Server", rf.me)
	rf.state = Follower
	rf.currentTerm = 0
	rf.votedFor = -1
