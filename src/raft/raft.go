...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex >= rf.logs.Len() {
		...
		if args.PrevLogIndex+1 == rf.logs.StartIndex {
			...
			// args.PrevLogIndex < rf.logs.StartIndex
			reply.Success = false
			reply.XIndex = args.PrevLogIndex
			reply.XTerm = args.PrevLogTerm
			return
...
func (rf *Raft) requestVoteRPCs() {
	...
	if len(rf.logs.Entries) == 0 {
		...
		args.LastLogTerm = rf.logs.GetTermByIndex(rf.logs.Len() - 1)
	}
	votes := 1
	n := len(rf.peers)
	for i := 0; i < n; i++ {
...
func (rf *Raft) checkCommit() {
	for N := rf.commitIndex + 1; N < rf.logs.Len(); N++ {
		if rf.logs.GetTermByIndex(N) != rf.currentTerm {
			continue
		}
		count := 1
		for i := 0; i < len(rf.peers); i++ {
